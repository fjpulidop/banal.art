...
No test framework is configured. Skip test runner check.
...
| Tool | Status | Notes |
|------|--------|-------|
| GitHub CLI | ok/missing | gh auth status |
| OpenSpec | ok | ... |
| Dependencies | ok | npm install |
| Test runner | N/A | No test framework configured |
```
...
**If the user passed issue references** (e.g. `#85, #71`):
- Fetch each issue:
  ```bash
...
- Tags for each task: `[fullstack]`, `[api]`, `[client]`, `[lib]`, `[auth]`
...
All tasks route to the `developer` agent (full-stack) since this is a single-layer Next.js app.
...
5. Create PR (if GitHub CLI is available):
   ...
   ---
   Generated by `/implement` pipeline
   EOF
   )"
   ```
   If `gh` is not authenticated, print a compare URL for manual PR creation.

#### Backlog updates (Read & Write mode)

- For fully resolved issues: add a comment noting completion and reference the PR (if created):
  ```bash
...
- For partially resolved issues: add a comment noting progress:
  ```bash
  gh issue comment {number} --body "Partial progress: {description}. Remaining: {items}."
  ```

### 4d. Monitor CI

**Only if code was pushed.**
