...
- Tag each task with its layer: `[fullstack]`, `[api]`, `[client]`, `[lib]`, `[auth]`

### 4. Respect the Architecture

This project follows this architecture:
```
banal.art — Next.js 16 fullstack app

src/
├── app/                    # Next.js App Router
│   ├── page.tsx           # Main client component (upload, display, share)
│   ├── layout.tsx         # Root layout with fonts
│   ├── globals.css        # Tailwind v4 + CSS variables
│   ├── login/page.tsx     # Login page
│   └── api/
│       ├── critique/route.ts   # Claude vision API → art critique
│       ├── tts/route.ts        # ElevenLabs TTS → audio
│       └── auth/[...nextauth]/ # NextAuth route handler
├── lib/
│   ├── canvas.ts          # Museum plaque image generation (Canvas API)
...
- Next.js App Router patterns (route handlers, `"use client"` directive)
- Tailwind CSS v4 with CSS variables (e.g., `var(--font-serif)`, `var(--accent)`)
- No database — stateless API routes with in-memory rate limiting
- No test framework configured — flag when tests would be valuable
...
- No git remote configured yet — don't assume GitHub is available
- Rate limiting is in-memory (resets on restart) — keep this in mind for scaling discussions
